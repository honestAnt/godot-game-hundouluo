...
var max_level = 5
var level_scenes = {
	1: "res://src/scenes/levels/level1.tscn",
...
func _ready():
	# 初始化游戏
	process_mode = Node.PROCESS_MODE_ALWAYS  # 允许在暂停时继续处理
...
func load_level(level_number):
	if level_scenes.has(level_number):
		...
		if level_node.has_signal("level_completed"):
			if not level_node.is_connected("level_completed", _on_level_completed):
				level_node.connect("level_completed", _on_level_completed)
	else:
...
func start_game():
	...
	if has_node("/root/AudioManager"):
		get_node("/root/AudioManager").play_music("level_1")
# 输入处理
func _input(event):
	# 按ESC键暂停/继续游戏
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_ESCAPE and (current_state == GAME_STATE.PLAYING or current_state == GAME_STATE.PAUSED):
			toggle_pause()
